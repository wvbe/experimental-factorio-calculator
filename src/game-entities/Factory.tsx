...
import { isRecipeCompatibleWithBuilding } from '../functions/helpers';
...
export const Building: FunctionComponent<BuildingProps> = ({ building, input = [], recipe }) => {
	...
	const isRecipeCompatible = recipe ? isRecipeCompatibleWithBuilding(recipe, building) : false;
	const table = {
		Recipe: recipe ? (
			<Recipe recipe={recipe} hasWarning={!isRecipeCompatible} />
		) : (
			<em>No recipe</em>
		),
